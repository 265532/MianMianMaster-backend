...
## 阶段五：社区与社交互动 (Community)（已提前至阶段四前完成）

### 1. 面试复盘社群
- **目标**：实现“面试复盘社群”功能。
- **任务拆解**：
  - [x] 设计 `Post`（帖子/文章）、`Comment`（评论）模型。
  - [x] 支持将个人的面试报告/错题一键分享为匿名或实名帖子（基础数据结构已完成，预留了 LLM 分析字段）。
  - [x] 设计 `UserFollow`（用户关注）和 `PostLike`（点赞）模型。
  - [x] 开发社区流（Feed）接口及搜索接口。
