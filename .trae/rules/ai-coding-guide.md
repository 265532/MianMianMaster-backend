...
- **模块交接文档 (Mandatory)**: 每次完成特定业务模块的开发、重构或 Debug 之后，**必须**在 `docs/` 目录下（或该模块专属的子目录中，如 `docs/auth/`、`docs/business/`）生成或更新一份该模块的**交接文档**（如 `handover.md`）。文档内容需包含：当前模块的核心实现思路、未解决的问题（Todo）、关键的依赖关系、以及下一步的开发建议。这有助于后续 AI 会话快速恢复上下文。
